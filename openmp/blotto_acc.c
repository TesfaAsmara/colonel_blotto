...
void play_game(int i, int j, int* player1, int* player2, int numCastles, int numTroops) {
    ...
    // Print the output to screen instead of filenames
    // Remove .txt
    printf("results/C%02d_T%03d/%d_%d_%d\n", numCastles, numTroops, i, j, result);
...
void tournament(int numCastles, int numTroops) {
    ...
    int strategies2[MAX_CONFIGURATIONS][numCastles];
    ...
    while (fgets(line, sizeof(line), configurations_file)) {
        ...
        while (token != NULL && index < MAX_CONFIGURATIONS) {
            ...
            strategies2[count][index++] = atoi(token);
            token = strtok(NULL, ",");
        }
        count++;
    }
    ...
    FILE* results_file = fopen("output_acc.txt", "w");
    ...
    int results[count][count];
    #pragma acc parallel loop independent copyin(strategies[0:count][numCastles]) copyout(results[0:count][0:count])
    for (int i = 0; i < count; i++) {
        // if (i==0) printf("Num threads: %d \n", omp_get_num_threads());
        for (int j = i+1; j < count; j++) {
            // printf("%d_%d_%d \n", i,j, game(strategies[i], strategies[j], numCastles));
            results[i][j] = game(strategies[i], strategies[j], numCastles);
            // if (omp_get_thread_num()==0) {
                // fprintf(results_file0, "%d_%d_%d \n", i,j, game(strategies[i], strategies[j], numCastles));
            // }
            // else if (omp_get_thread_num()==1) {
            //     fprintf(results_file1, "%d_%d_%d \n", i,j, game(strategies2[i], strategies2[j], numCastles)); 
            // }
